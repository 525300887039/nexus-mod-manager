//! Mod 扫描、启停、卸载、安装、备份与还原。
//!
//! 本模块无 Tauri 依赖。GUI 侧 dialog 选路径副作用由 src-tauri 薄壳负责，
//! core 仅暴露纯路径输入的业务函数。
...
/// 从 GameProfile 中提取 mods 子目录名（带默认值 "mods"）。
pub fn current_mods_subdir(profile: &GameProfile) -> String {
    let trimmed = profile.mods_subdir.trim();
    if trimmed.is_empty() {
        "mods".to_string()
    } else {
        trimmed.to_string()
...
pub fn get_mods_dir(game_path: &str, mods_subdir: &str) -> PathBuf {
...
pub fn scan_mods_internal_with_subdir(game_path: &str, mods_subdir: &str) -> Vec<ModInfo> {
...
/// 切换 mod 的启用 / 禁用：移动 mod 文件夹（或 flat mod 的所有相关文件）到对应目录。
/// 成功时返回最新的扫描结果；失败返回 `ModResult` 含 error。
pub fn toggle_mod(game_path: &str, mods_subdir: &str, mod_info: &ToggleModInfo) -> ModResult {
    let mods_dir = get_mods_dir(game_path, mods_subdir);
    let disabled_dir = get_disabled_dir(game_path);
    ...
    if mod_info.is_folder {
        ...
        let files = mod_info.files.clone().unwrap_or_default();
...
pub fn smart_extract_archive(archive_path: &str, mods_dir: &Path) -> Result<(), String> {
...
/// 把传入的路径列表（文件夹 / 压缩包混合）逐个安装到 `mods_dir`。
/// 返回每个已成功处理路径的 file_name 字符串列表；任一项失败立即返回 Err。
pub fn install_paths_to_mods_dir(
    file_paths: &[String],
    mods_dir: &Path,
...
/// 把整个 mods 目录打包成 ZIP 到 `zip_path`。
pub fn create_zip_from_mods_dir(mods_dir: &Path, zip_path: &str) -> Result<(), String> {
    ...
    add_dir_to_zip(&mut zip_writer, mods_dir, mods_dir, options)?;
