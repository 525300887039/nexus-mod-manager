//! Nexus Mods REST API + GraphQL 客户端。
//!
//! 本模块无 Tauri 依赖。所有持状态的函数接收 `&AppContext`。
//! GUI / CLI 调用方负责提供 tokio runtime 驱动 async 函数。

use crate::config;
use crate::context::AppContext;
use crate::db;
use crate::mods;
pub use crate::types::nexus::NexusModInfo;
...
/// 判断当前账号是否为 Premium 会员，带 AppContext 懒缓存。
/// 未配置 API Key 时返回 Ok(false)（视为非会员，走 webview 下载路径）。
/// validate 接口失败（网络异常、Key 失效）时返回 Err，由调用方回退处理。
pub async fn ensure_premium_status(ctx: &AppContext) -> Result<bool, String> {
    {
        let cache = ctx
            .nexus_is_premium
            .lock()
    ...
    if let Ok(mut cache) = ctx.nexus_is_premium.lock() {
...
fn cache_nexus_mods(
    ...
    cache_nexus_mods_in_memory(ctx, game_domain, mods)?;

    let db_conn = ctx
        .db
        .lock()
        .map_err(|e| format!("数据库锁已损坏: {}", e))?;
    db::nexus_mod_cache_upsert_db(&db_conn, game_domain, mods)?;
...
fn read_cached_nexus_mods(
    ...
    let cached_mods = {
        let db_conn = ctx
            ...
            .lock()
            .map_err(|e| format!("数据库锁已损坏: {}", e))?;
        db::nexus_mod_cache_load_db(&db_conn, game_domain)?
    };

    if !cached_mods.is_empty() {
        cache_nexus_mods_in_memory(ctx, game_domain, &cached_mods)?;
    }
...
fn read_cached_nexus_mods_by_ids(
    ...
    if let Some(max_age_millis) = max_age_millis {
        let db_cached = {
            let db_conn = ctx
                ...
                .lock()
                .map_err(|e| format!("数据库锁已损坏: {}", e))?;
            db::nexus_mod_cache_get_many_db(&db_conn, game_domain, mod_ids, Some(max_age_millis))?
        };

        if !db_cached.is_empty() {
            let mods_to_hydrate = db_cached.values().cloned().collect::<Vec<_>>();
            cache_nexus_mods_in_memory(ctx, game_domain, &mods_to_hydrate)?;
        }
    ...
    let db_cached = {
        let db_conn = ctx
            ...
            .lock()
            .map_err(|e| format!("数据库锁已损坏: {}", e))?;
        db::nexus_mod_cache_get_many_db(&db_conn, game_domain, &missing_ids, None)?
    };
...
fn find_local_mod(name: &str, ctx: &AppContext) -> Option<mods::ModInfo> {
    let game_path = ctx.game_path.lock().ok()?.clone()?;
...
async fn fetch_mod_by_id(
    ...
    let api_key = get_saved_api_key()?;
    fetch_mod_by_id_with_key(mod_id, game_domain, ctx, &api_key).await
}
...
async fn fetch_mod_by_id_with_key(
    ...
    .await?;
    cache_nexus_mods(ctx, game_domain, &[mod_info.clone()])?;
    Ok(mod_info)
}
...
async fn ensure_cached_mod_lists(ctx: &AppContext, game_domain: &str) -> Result<(), String> {
    let cached_mods = read_cached_nexus_mods(ctx, game_domain)?;
    if !cached_mods.is_empty() {
        return Ok(());
    }
    ...
    cache_nexus_mods(ctx, game_domain, &collected)
}
...
async fn resolve_recently_updated_page_items(
    ...
    let fallback_cached_mods = if force_refresh {
        ...
        read_cached_nexus_mods_by_ids(ctx, game_domain, page_mod_ids, None)?
    };
    ...
    for mod_id in page_mod_ids {
        ...
        match fetch_mod_by_id_with_key(*mod_id, game_domain, ctx, api_key).await {
...
// ───────────────────────── 9 个 IPC 命令对应的 pub async fn ─────────────────────────

pub async fn validate_key(key: &str) -> Result<NexusValidateResult, String> {
    let key = key.trim();
    nexus_get("/users/validate.json", key).await
}

pub async fn get_trending(ctx: &AppContext) -> Result<Vec<NexusModInfo>, String> {
    ...
    .await?;
    cache_nexus_mods(ctx, &game_domain, &mods)?;
    Ok(mods)
}

pub async fn get_latest_added(ctx: &AppContext) -> Result<Vec<NexusModInfo>, String> {
    let game_domain = current_game_domain(ctx)?;
    let api_key = get_saved_api_key()?;
    let mods: Vec<NexusModInfo> = nexus_get(
        &format!("/games/{}/mods/latest_added.json", game_domain),
...
pub async fn get_latest_updated(ctx: &AppContext) -> Result<Vec<NexusModInfo>, String> {
    let game_domain = current_game_domain(ctx)?;
    let api_key = get_saved_api_key()?;
    let mods: Vec<NexusModInfo> = nexus_get(
        &format!("/games/{}/mods/latest_updated.json", game_domain),
...
pub async fn get_recently_updated_page(
    ctx: &AppContext,
    period: &str,
    page: u64,
    page_size: u64,
...
pub async fn get_mod(ctx: &AppContext, mod_id: u64) -> Result<NexusModInfo, String> {
    let game_domain = current_game_domain(ctx)?;
    fetch_mod_by_id(mod_id, &game_domain, ctx).await
}

pub async fn get_mod_files(ctx: &AppContext, mod_id: u64) -> Result<Vec<NexusFileInfo>, String> {
    let game_domain = current_game_domain(ctx)?;
    get_mod_files_raw(&game_domain, mod_id).await
}

pub async fn find_mod_by_name(
    ctx: &AppContext,
    name: &str,
    ...
    let Some(local_mod) = find_local_mod(lookup, ctx) else {
        return Ok(None);
    };
    let game_domain = current_game_domain(ctx)?;
    ...
    if let Some(nexus_id) = local_mod.nexus_id {
        ...
        match fetch_mod_by_id(nexus_id, &game_domain, ctx).await {
    ...
    if let Err(error) = ensure_cached_mod_lists(ctx, &game_domain).await {
