//! 翻译通道 2：通用 OpenAI-兼容 LLM 协议（POST `/chat/completions`）。
//!
//! 含 `LlmConfig` 持久化（`llm_config.json`）和异步 `translate` 函数。
...
fn config_dir() -> Result<PathBuf, String> {
    Ok(paths::writable_config_dir())
}

fn config_path() -> Result<PathBuf, String> {
    Ok(paths::current_config_file("llm_config.json"))
}

fn load_config_path() -> Option<PathBuf> {
    let current = paths::current_config_file("llm_config.json");
    if current.exists() {
        return Some(current);
    }

    paths::existing_config_file("llm_config.json")
}
...
pub fn sanitize_for_save(mut config: LlmConfig) -> LlmConfig {
...
pub fn load_config() -> LlmConfig {
    ...
    if path != paths::current_config_file("llm_config.json") {
...
pub fn resolve_chat_completions_url(api_url: &str) -> Result<String, String> {
